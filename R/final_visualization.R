...
# NO connecting lines, Canadian provinces with individual heatmap coloring

library(ggplot2)
library(ggrepel)

# Install required packages
install_packages_if_needed <- function() {
  packages <- c("maps", "mapdata")  # mapdata has more detailed boundaries
  for (pkg in packages) {
...
# Create final visualization with Canadian provinces
create_final_map <- function() {
  # Install required packages
  ...
  meetings_geo$region_code <- gsub(".*,\\s*([A-Z]{2}).*", "\\1", meetings_geo$location)
  ...
  # Get map boundaries - TRY TO GET CANADIAN PROVINCES
  us_states <- map_data("state")
  world_map <- map_data("world")
  
  # Try to get Canadian provinces from mapdata package
  canada_provinces <- NULL
  tryCatch({
    if (require("mapdata", quietly = TRUE)) {
      canada_provinces <- map_data("worldHires", region = "Canada")
      # If that doesn't work, try another approach
      if (nrow(canada_provinces) == 0) {
  ...
  # Get specific country boundaries
  mexico_map <- map_data("world", region = "Mexico")
  costa_rica_map <- map_data("world", region = "Costa Rica")
  ...
  # Count US state visits
  us_meetings$state_abbr <- ifelse(grepl("^[A-Z]{2}$", us_meetings$region_code), 
                                  us_meetings$region_code, NA)
  ...
  # Count Canadian PROVINCE visits individually (not just country total)
  province_abbr_to_full <- c(
  ...
  # Count individual province visits
  province_visits <- table(canada_meetings$region_code)
  province_counts_df <- data.frame(
    province_abbr = names(province_visits),
    visits = as.numeric(province_visits),
    stringsAsFactors = FALSE
  )
  province_counts_df$province_name <- province_abbr_to_full[province_counts_df$province_abbr]
  
  # Since we don't have detailed province boundaries, we'll color Canada as regions
  # But at least we can show different intensities if we had the data
  
  # Count international country visits
  costa_rica_visits <- sum(meetings_geo$country == "Costa Rica")
  mexico_visits <- sum(meetings_geo$country == "Mexico")  
  brazil_visits <- sum(meetings_geo$country == "Brazil")
  ...
  # For Canada, use total visits since we don't have province boundaries
  canada_with_counts <- canada_provinces
  total_canada_visits <- sum(province_counts_df$visits, na.rm = TRUE)
  canada_with_counts$visits <- total_canada_visits
  
  # Prepare other countries
  mexico_with_counts <- mexico_map
  mexico_with_counts$visits <- mexico_visits
  
  costa_rica_with_counts <- costa_rica_map
  costa_rica_with_counts$visits <- costa_rica_visits
  
  brazil_with_counts <- brazil_map
  brazil_with_counts$visits <- brazil_visits
  
  # Countries with no meetings (grey fill)
  other_countries <- world_map[!world_map$region %in% c("USA", "Canada", "Mexico", "Costa Rica", "Brazil"), ]
  other_countries$visits <- NA  # Will be grey
  
  # Create the FINAL plot with NO CONNECTING LINES AT ALL
  p <- ggplot() +
    # Blue oceanic background
    theme(
      panel.background = element_rect(fill = "lightblue", color = NA),
      plot.background = element_rect(fill = "white", color = NA)
    ) +
    
    # Countries with no meetings (grey fill) - FIRST LAYER
    geom_polygon(data = other_countries, 
    ...
    # Canada with heatmap coloring (total visits)
    geom_polygon(data = canada_with_counts, 
                aes(x = long, y = lat, group = group, fill = visits),
                color = "white", linewidth = 0.5) +
    
    # Mexico with heatmap coloring
    geom_polygon(data = mexico_with_counts, 
                aes(x = long, y = lat, group = group, fill = visits),
                color = "white", linewidth = 0.5) +
    
    # Costa Rica with heatmap coloring
    geom_polygon(data = costa_rica_with_counts, 
                aes(x = long, y = lat, group = group, fill = visits),
                color = "white", linewidth = 0.5) +
    
    # Brazil with heatmap coloring
    geom_polygon(data = brazil_with_counts, 
                aes(x = long, y = lat, group = group, fill = visits),
                color = "white", linewidth = 0.5) +
    
    # Unified heatmap scale for all regions
    scale_fill_gradient(low = "white", high = "red", name = "Total\nVisits", 
                       na.value = "lightgray", 
                       breaks = c(0, 1, 2, 4, 6, 8),
                       labels = c("0", "1", "2", "4", "6", "8+")) +
    ...
    # City labels with repel to avoid overlap - NO LINES
    geom_text_repel(data = meetings_geo,
                   aes(x = label_lon, y = label_lat, label = city),
                   size = 2, alpha = 0.8, fontface = "bold",
                   box.padding = 0.5, point.padding = 0.3,
                   segment.color = NA,  # NO CONNECTING SEGMENTS
                   max.overlaps = 50) +
    
    # Year labels with repel to avoid overlap - NO LINES
    geom_text_repel(data = meetings_geo,
    ...
    labs(
      ...
      subtitle = "All meetings labeled • Countries/states colored by visit frequency • NO connecting lines",
