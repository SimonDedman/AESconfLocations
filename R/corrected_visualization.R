...
# Fixed border slicing, proper country/province coloring, consistent heatmap
...
# Create corrected comprehensive visualization
create_corrected_map <- function() {
  # Install maps package if needed
  ...
  # Count Canadian province visits
  canada_province_visits <- table(canada_meetings$region_code)
  canada_counts_df <- data.frame(
    province_abbr = names(canada_province_visits),
    visits = as.numeric(canada_province_visits),
    stringsAsFactors = FALSE
  )
  ...
  # Prepare other regions with visit counts
  canada_with_counts <- canada_map
  total_canada_visits <- sum(canada_counts_df$visits, na.rm = TRUE)
  ...
  other_countries$visits <- 0
  
  # Create the corrected plot (NO CONNECTING LINES to prevent border slicing)
  p <- ggplot() +
    ...
    geom_text_repel(data = meetings_geo,
                   ...
                   segment.color = "navy", segment.size = 0.2,
                   max.overlaps = 50) +
    ...
    labs(
      ...
      subtitle = "All meetings labeled • Countries/states/provinces colored by visit frequency • Grey = no meetings",
